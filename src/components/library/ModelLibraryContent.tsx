...
import { useModels } from '@/lib/hooks/use-library';
import { LibraryItemCard } from './LibraryItemCard';
import { SingleDocFormDialog } from './SingleDocFormDialog';

export function ModelLibraryContent() {
  const t = useTranslations('library.models');
  const tl = useTranslations('library');
  const tc = useTranslations('common');
  const { items: modelItems, isLoading, isError, mutate } = useModels();
  ...
  const handleCreate = async () => {
    ...
    try {
      const res = await fetch('/api/library/models', {
        method: 'POST',
  ...
  const handleDelete = async (id: string) => {
    try {
      const res = await fetch(`/api/library/models/${id}`, { method: 'DELETE' });
      if (!res.ok) throw new Error();
      mutate();
    } catch {
      // silent
      ...
      <div className="mx-auto max-w-7xl px-6 py-8">
        ...
        {isLoading ? (
          ...
          <div className="mt-8 flex flex-col items-center rounded-2xl border-2 border-dashed border-gray-200 py-16">
            ...
            <p className="mt-4 text-gray-500">{t('noModels')}</p>
            <button
              onClick={() => setIsCreating(true)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative z-10 w-full max-w-md rounded-2xl border border-gray-200 bg-white shadow-xl">
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div>
                <label htmlFor="model-desc" className="block text-sm font-medium text-gray-700">
                  {t('modelDescription')}
                </label>
                <input
                  id="model-desc"

...
│   │   │   ├── library/               # Libraries (reusable cache)
│   │   │   │   ├── page.tsx           # Library overview
│   │   │   │   ├── templates/         # Template library
│   │   │   │   ├── models/            # Model library
│   │   │   │   └── entities/          # Entity (company) library
...
│   │       ├── library/               # Library CRUD (templates, models, entities)
...
│   │   │   ├── ProjectCard.tsx
│   │   │   ├── UploadZone.tsx         # Upload with drag & drop
│   │   │   ├── RoleTagDropdown.tsx   # Per-file role tag selector (Auto-detect default)
│   │   │   ├── LibraryPicker.tsx     # Select from library or upload new (tabbed UI)
...
│   │   │   │   ├── library-item.ts   # Library items (templates, models, entities)
...
|-------|------------|------------|----------|
| Extractor Agent | ✅ Extract + validate all factual data by entity (uses tag + content + filename) | ❌ Never touches | ❌ Never touches |
...
### Solution: Upload Tags + Content Detection + Multiple Suggestions

**Three layers, simple UI:**

**Layer 1 — Tag at Upload (user choice):**
Each uploaded source document has a small dropdown with a role tag. Default is "Auto-detect".

| Tag | When to use | Example documents |
|-----|------------|-------------------|
| Auto-detect (default) | Let AI decide from content + filename | Any document |
| Beneficiar | Documents about the project beneficiary | ID card, registration certificate, ONRC extract |
| Furnizor / Contractant | Documents about supplier or contractor | Supplier ID, technical offer, service contract |
| Subcontractant | Subcontractor documents | Subcontracting agreement, ID, experience declaration |
| Proiect | Documents describing the project overall | Specifications, feasibility study, HCL, DALI |
| Financiar | Financial documents | Cost estimates, summaries, financing schedule, budget |

**Layer 2 — Content + Filename Detection (Extractor Agent):**
When tag is "Auto-detect", Extractor Agent uses the original filename as hint (`CI_beneficiar_Popescu.pdf`) combined with content analysis ("Beneficiarul: SC X SRL") to assign roles automatically.

**Priority:** manual tag > content analysis > filename hint.

**Layer 3 — Multiple Suggestions (Writing Agent):**
When the agent CAN'T determine which entity a field belongs to, it generates one suggestion per possible entity and lets the user pick.

### UI Implementation
- Dropdown appears below each uploaded source file (compact, one click)
- Default: "Auto-detect" (grey text)
- When user selects a tag, the tag appears as a colored badge on the file card
- Tag is sent as metadata to Extractor Agent
...
interface EntityData {
  ...
  role_tag: string;        // 'auto' | 'beneficiar' | 'furnizor' | 'subcontractant' | 'proiect' | 'financiar'
  role_source: 'manual_tag' | 'content_detection' | 'filename_hint';  // How the role was determined
}
```

### How It Flows Through the Pipeline
1. **Extractor Agent**: receives tag + filename + content. Manual tag = absolute priority. Organizes data into entities. Unknown → `unassigned[]`
...
```
1. Extractor Agent (Sonnet 4.5)     → project_data.json (entity-based from tag + content + filename, validated)
...
## Libraries (Reusable Cache)

### Purpose
Libraries eliminate redundant AI processing. A consultant using the same template for 20 projects shouldn't pay for template analysis 20 times. Same for company data that doesn't change between projects.

### Three Library Types

| Library | Stores | Cached Output | Savings |
|---------|--------|---------------|---------|
| Templates | Uploaded DOCX templates | `template_schema.json` (fields, classification, hints) | Skip template analysis per project |
| Models | Completed model documents | `style_guide.json` (style, tone, rhetorical patterns, vocabulary) | Skip style analysis per project |
| Entities | Company/person documents (CI, CUI, ONRC) | `entity_data.json` (extracted data: CUI, address, representative, CAEN) | Skip entity extraction per project |

### MongoDB Schema — `library_items` Collection

```typescript
interface ILibraryItem extends Document {
  userId: string;
  type: 'template' | 'model' | 'entity';
  name: string;                    // User-friendly name ("Cerere finantare AFIR", "TANI DANONA SRL")
  originalFilename: string;
  fileHash: string;                // SHA256
  processedData: object;           // template_schema | style_guide | entity_data
  status: 'processing' | 'ready' | 'error';
  usageCount: number;              // How many projects use this
  lastUsedAt: Date;
}
...
### Project Flow with Libraries

```
New Project
  → Step 1: Template — pick from library OR upload new
  → Step 2: Model — pick from library OR upload new OR skip
  → Step 3: Entity — pick company from library OR upload new documents
  → Step 4: Source docs — upload ONLY project-specific documents
  → AI Processing — runs ONLY on what's new
```

### Critical: Copy on Use, Not Reference
When a project uses a library item, the `processedData` is **COPIED** into the project document. This means:
- Updating/deleting a library item does NOT affect existing projects
- Each project has its own snapshot of the data at time of creation
- User can modify copied data within a project without affecting the library

```typescript
// When user selects a library item for a project
const libraryItem = await LibraryItem.findById(selectedId);
await Project.findByIdAndUpdate(projectId, {
  $set: {
    [`libraryRefs.${libraryItem.type}`]: {
      libraryItemId: libraryItem._id,
      name: libraryItem.name,
      processedData: libraryItem.processedData,  // COPY, not reference
      copiedAt: new Date()
    }
  }
});
await LibraryItem.findByIdAndUpdate(selectedId, {
  $inc: { usageCount: 1 },
...
|-----------|-------------|
| **Add** | Upload document → AI processes → save `processedData` permanently |
| **View** | Show extracted data in user-friendly format (not raw JSON) |
| **Update** | Re-upload new version → re-process with AI → replace `processedData`. Existing projects keep old copy |
| **Delete** | Remove document + processedData. Existing projects keep their copy |

### UI Components
- **Library pages:** Sidebar menu → "Biblioteci" → 3 sub-pages (Templates, Models, Entities)
- **Library list:** Cards with name, upload date, last used date, usage count
- **Project creation:** Each upload step has tab "Alege din bibliotecă" next to "Încarcă nou"
- **Actuality confirmation:** When selecting entity from library, checkbox: "Confirm că datele firmei sunt actuale"

### Cost Impact
| Scenario | Without libraries | With libraries |
|----------|------------------|----------------|
| 1st project | $0.10-0.15 | $0.10-0.15 (same) |
| 2nd project, same company + template | $0.10-0.15 | $0.04-0.08 |
| 5th project, everything from library | $0.10-0.15 | $0.02-0.05 |
...
NEXT_PUBLIC_APP_URL=https://docfides.com
```
...
2. Create project + upload documents (source + template + optional 1-2 models — each selectable from library or uploaded new)
...
24. Reusable libraries (templates, models, entities) with cached AI processing and CRUD operations
